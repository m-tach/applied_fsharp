...
let run is = let sw = new System.Diagnostics.Stopwatch()
             ...
             printfn "\nExecuted %d instructions" executedInst

/// run and return amount of executed instructions and time elapsed im [ms]
let runBenchmark is = let sw = System.Diagnostics.Stopwatch()
                      executedInst <- 0
                      let _  = sw.Start()
                      let stk = runArgs is [||]
                      let elabsed = int sw.ElapsedMilliseconds
