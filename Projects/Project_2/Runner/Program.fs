...
let runInstrs instrs =
    Array.toList(VirtualMachine.execcode(List.toArray (Machine.code2ints instrs), (Array.zeroCreate 20000), (Array.zeroCreate 0)) false)

let runAST ast =
    let instrs = CP ast
    let optiInstrs = optimize instrs
    printfn "%s" (String.concat ", " (List.map(fun x -> string(x)) (runInstrs instrs)))
    printfn "%s" (String.concat ", " (List.map(fun x -> string(x)) (runInstrs optiInstrs)))
