...
module ASTConverter =
    ...
    and decsToNodes decs = match decs with
                           | [dec] -> decToNode dec
    ...
    and stmsToNodes stms = match stms with
                           | [stm] -> stmToNode stm
    ...
    and guardToNodeList (GC(stuff)) = List.map(fun (x, y) -> Node("guarded", [expToNode x; stmsToNodes y])) stuff